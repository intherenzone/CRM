...
$('#id_account').change(function(){
    ...
    var Account= $("#id_account").val()
        $.get("/activities/contacts/", {"Account":Account, "csrfmiddlewaretoken": csrftoken}, function(data){
          $("#id_contacts").html("")
...
$("#comment_edit").click(function(e){
    ...
    $.ajax({
        url : "/oppurtunities/comment_edit/",
        type : "POST",
        data : formData,
        cache: false,
        contentType: false,
...
function remove_comment(x){
  ...
  if (warn == true){
    $.post('/oppurtunities/comment_remove/', {"comment_id":x, "csrfmiddlewaretoken": csrftoken, }, function(data){
      if(data.error){
        alert(data.error)
      } else {
